...
class ButtonInput(Enum):
    ROBOT_CUBE1 = 'robot_cube1'
    ROBOT_CUBE2 = 'robot_cube2'
...
class TeleopPlanner:
    ...
    def _update(self, timer_event):
        ...
        if self._mode == Mode.AUTO and is_planner_cmd_valid:
            ...
            cmd.mode_force = False
        elif self._mode == Mode.FORCE and is_joy_cmd_valid:
            cmd = self._joy_cmd
            cmd.mode_force = True

        if self._robot == Robot.CUBE1 and is_planner_cmd_valid:
            cmd = self._planner_cmd
            # cmd.robot_cube1 = True
            cmd.robot_cube2 = False
        elif self._robot == Robot.CUBE2 and is_joy_cmd_valid:
            cmd = self._joy_cmd
            # cmd.robot_cube1 = False
            cmd.robot_cube2 = True
