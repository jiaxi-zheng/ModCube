...
#include <numeric>
#include <ros/ros.h>
#include <modcube_msgs/FluidDepth.h>
#include <modcube_msgs/NavigationState.h>
#include <modcube_msgs/TeledyneDvlData.h>
#include <modcube_msgs/XsensImuData.h>
#include <tf/transform_broadcaster.h>
#include <modcube_alarms/alarms.h>
#include <modcube_alarms/alarm_client.h>
#include <modcube_msgs/SetPose.h>
#include "ekf.h"
...
StateEstimator::StateEstimator(ros::NodeHandle& n, ros::NodeHandle &pn) : n(n), pn(pn), alarm_client(n)
{
  ...
  this->navigation_state_pub = n.advertise<modcube_msgs::NavigationState>("gnc/navigation_state", TOPIC_QUEUE_SIZE);
  ...
  this->alarm_client.clear(modcube_alarms::AlarmType::STATE_ESTIMATION_NOT_INITIALIZED, "State estimation initialized.");
}
...
void StateEstimator::update(const ros::TimerEvent& e)
{
  ...
  if (this->delayed_queue.size() > (unsigned) this->max_delayed_queue_size) {
      this->alarm_client.set(modcube_alarms::AlarmType::STATE_ESTIMATION_DELAYED, "Delayed queue is too large.");
  } else {
      this->alarm_client.clear(modcube_alarms::AlarmType::STATE_ESTIMATION_DELAYED, "Delayed queue is acceptable.");
  }
...
void StateEstimator::handle_imu(const modcube_msgs::XsensImuData::ConstPtr& msg)
{
...
void StateEstimator::handle_dvl(const modcube_msgs::TeledyneDvlData::ConstPtr& msg)
{
  if (!this->is_initialized) {
    this->is_initialized = true;
    this->last_evaluation_time = msg->header.stamp - this->horizon_delay;
...
void StateEstimator::handle_depth(const modcube_msgs::FluidDepth::ConstPtr& msg)
{
  if (!this->is_initialized) {
    this->is_initialized = true;
    this->last_evaluation_time = msg->header.stamp - this->horizon_delay;
...
bool StateEstimator::handle_set_pose(
  modcube_msgs::SetPose::Request &req,
  modcube_msgs::SetPose::Response &res)
{
...
{
  modcube_msgs::NavigationState msg;
...
StateEstimator::ImuMsg::ImuMsg(const modcube_msgs::XsensImuData::ConstPtr &msg) {
...
StateEstimator::DvlMsg::DvlMsg(const modcube_msgs::TeledyneDvlData::ConstPtr &msg) {
  this->stamp = msg->header.stamp;
...
StateEstimator::DepthMsg::DepthMsg(const modcube_msgs::FluidDepth::ConstPtr &msg) {
  this->stamp = msg->header.stamp;
