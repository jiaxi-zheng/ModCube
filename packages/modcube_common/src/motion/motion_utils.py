...
from modcube_util.transforms import twist_body_to_world, quat_to_rpy
...
import typing
from modcube_msgs.msg import TrajPoint
from modcube_msgs.srv import SetPose, SetPoseRequest, SetPoseResponse
...
class MotionUtils:
    ...
    def _pub_target(self, timer_event):
        ...
        # print(" twist : ", twist)
        euler = Vector3()
        orientation_euler = quat_to_rpy(pose.orientation)
        euler.x = orientation_euler[0]
        euler.y = orientation_euler[1]
        euler.z = orientation_euler[2]
        # print(" orientation_euler : ", orientation_euler)

        if pose is None or twist is None:
            return

        tp = TrajPoint(pose, twist, euler)
