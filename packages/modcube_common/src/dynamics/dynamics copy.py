...
class Dynamics:
    ...
    def compute_tau(self, eta, v, v_d):
        ...
        tau_desired = np.dot(M, v_d) + np.dot(C, v) + np.dot(D, v) + G.T
        tau_impedance = np.dot(1, (eta - 0)) + np.dot(1, (v - v_d))

        # Total control input = Desired control input + Impedance control input
        total_tau = tau_desired + tau_impedance

        # Calculate the external force by subtracting the desired force from the total force
        external_force = tau_impedance
