...
class TeleopMission:
    ...
    def _handle_goto(self, args):
        ...
        roll_matrix = SO3.Rx(args.roll)
        pitch_matrix = SO3.Ry(args.pitch)
        yaw_matrix = SO3.Rz(args.yaw)

        combined_rotation = yaw_matrix* pitch_matrix* roll_matrix #(ZYX)
        # combined_rotation = roll_matrix* pitch_matrix* yaw_matrix #(ZYX)
        translation_vector = np.array([args.x, args.y, args.z])
        pose = SE3.Rt(combined_rotation, translation_vector)
        # pose = SE3.Rt(SO3.Rx(args.yaw), np.array([args.x, args.y, args.z]))

        # print("SO3.Rx(args.yaw) : ")
        # print(SO3.Rx(args.yaw))
        # print("pose : ")
        # print(pose)
        # print("#########################")
